...
PS_OPEN_FUNC(redis)
{
    ...
    for (i=0,j=0,path_len=strlen(save_path); i<path_len; i=j+1) {
        ...
        if (i < j) {
            ...
            RedisSock *redis_sock;
            if(url->host) {
                redis_sock = redis_sock_create(url->host, strlen(url->host), url->port, NULL, 0, timeout, persistent, persistent_id, retry_interval, 0);
            } else { /* unix */
                redis_sock = redis_sock_create(url->path, strlen(url->path), 0, NULL, 0, timeout, persistent, persistent_id, retry_interval, 0);
            }

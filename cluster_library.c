...
static void cluster_node_add_slot_range(redisClusterNode *node, unsigned short low,
								unsigned short high) {
...
static int cluster_map_slots(redisCluster *c, clusterReply *r) {
    ...
    for(i=0;i<r->elements;i++) {
		...
		cluster_node_add_slot_range(master, low, high);
    }
...
cluster_init_seeds(redisCluster *cluster, HashTable *ht_seeds) {
    ...
    /* Grab our seeds in a randomized order */
    z_seeds = cluster_shuffle_seeds(ht_seeds, &count);

    /* Iterate over seeds array */
    for (i = 0; i < count; i++) {
        z_seed = z_seeds[i];

        /* Make sure seed is non null and a string */
